...
const pageShell = ({ title, eyebrow, body, sidebar = '', className = '', description = title, rail = true }) => `<!doctype html>
...
<div class="page-layout ${className}">${sidebar ? `<aside class="side-nav">${sidebar}</aside>` : ''}<main class="page-main" id="main" tabindex="-1">${rail ? sectionRail(body.replace(/<div id="lab-texts"[\s\S]*?<\/div>/, ' ')) : ''}<p class="eyebrow">${escape(eyebrow)}</p>${body}</main></div>
...
const quickTrainer = ({ scope, seed, items, eyebrow, title, lead, doneText, footerHref, footerText }) => {
  if (items.length !== 2) throw new Error(`Expected two quick-check slots for ${scope}`);
  const questions = items.map((variants, questionIndex) => {
    if (!variants.length) throw new Error(`Empty question pool for ${scope}`);
    const slotId = `Q${scope}-${questionIndex}`;
    // Все варианты слота лежат на странице сразу: подмена вопроса после промаха
    // не должна требовать сети — учебник обязан работать офлайн.
    const blocks = variants.map((item, variantIndex) => {
      const variantId = `${slotId}/${variantIndex}`;
      const shift = (seed + questionIndex + variantIndex) % item.options.length;
      ...
      const labels = options.map((option, optionIndex) => `<label><input type="radio" name="trainer-${variantId}" value="${optionIndex}"><span>${escape(option)}</span></label>`).join('');
      return `<div class="trainer-options" data-trainer-variant="${variantIndex}" data-answer="${answerDigest(variantId, answer)}" data-stem="${escape(item.stem)}" data-explanation="${escape(item.explanation)}"${variantIndex ? ' hidden' : ''}>${labels}</div>`;
    }).join('');
    return `<fieldset class="trainer-question" data-trainer-question="${slotId}"><legend><span class="trainer-num">${questionIndex + 1}</span><span data-trainer-stem>${escape(variants[0].stem)}</span></legend>${blocks}<div class="trainer-actions"><button type="button" data-trainer-check>Проверить</button><p class="trainer-feedback" data-trainer-feedback hidden role="status"></p></div></fieldset>`;
  }).join('');
...
const referenceCards = `<div class="reference-cards"><a href="/reference/archive/"><span>Аттестация</span><strong>Экзаменационный банк и рубрики</strong></a><a href="/assessment/"><span>Интерактив</span><strong>Автоматическая проверка</strong></a><a href="/curriculum/"><span>Навигация</span><strong>Все 34 главы курса</strong></a><a href="/book/"><span>Печать и PDF</span><strong>Вся книга одной страницей</strong></a></div>`;
...
.prose pre:focus-visible,.prose table:focus-visible{outline:2px solid var(--teal);outline-offset:2px}
...
/* Книга одной страницей. На экране она читается как обычная глава, на бумаге
   каждая глава начинается с новой страницы, а листинги, таблицы и схемы не
   разрываются пополам. Адрес внешней ссылки печатается рядом: на листе сама
   ссылка бесполезна, а сноска с адресом — нет. Внутренние ссылки остаются
   местными и адрес не получают, иначе каждая из тысяч перекрёстных ссылок
   тянула бы за собой строку. */
.book-page .catalog-head{max-width:900px;margin:0 auto 46px}.book-page .page-main>.eyebrow{max-width:900px;margin:0 auto 12px}
.book-prose h1{margin-top:64px}
.book-prose h1:first-of-type{margin-top:0}
@media print{
 .book-prose h1{break-before:page;font-size:26pt}
 .book-prose h1:first-of-type{break-before:auto}
 .book-prose h2,.book-prose h3{break-after:avoid}
 .book-prose pre,.book-prose table,.book-prose figure,.book-prose blockquote{break-inside:avoid}
 .book-prose p,.book-prose li{orphans:2;widows:2}
 .book-prose pre{border-left-width:2px;background:none;color:inherit;border:1px solid #999}
 .book-prose figure.scheme{overflow:visible}
 .book-prose figure.scheme svg{min-width:0}
 .book-prose a[href^="http"]::after{content:" (" attr(href) ")";font-size:.82em;color:#555;word-break:break-all}
 .skip-link,.read-progress,.search-overlay,.define-card{display:none!important}
}
/* --- Телефоны -------------------------------------------------------------
...
const recordQuickCheck=(id,correct,variant)=>{const old=quickRecords[id];quickRecords[id]={correct,attempts:(old?.attempts||0)+1,last:Date.now(),variant};saveQuick()};
...
const answerIndex=(salt,box)=>{const digest=box.dataset.answer,total=box.querySelectorAll('input[type=radio]').length;for(let index=0;index<total;index+=1)if(answerDigest(salt,index)===digest)return index;return -1};
...
/* Мини-тренажёр. Слот — пул смежных вопросов об одном и том же утверждении;
   неверный ответ меняет вопрос внутри слота и не показывает ни верный вариант,
   ни разбор. Раньше промах подсвечивал верный ответ и печатал разбор, и кнопка
   «Проверить ещё раз» не значила ничего: оставалось повторить увиденное.
   Показанный вариант выбирается по числу попыток, поэтому переживает
   перезагрузку и не требует своего ключа хранилища. */
document.querySelectorAll('[data-chapter-trainer]').forEach(trainer=>{
 const questions=[...trainer.querySelectorAll('[data-trainer-question]')],score=trainer.querySelector('[data-trainer-score]'),summary=trainer.querySelector('[data-trainer-summary]');
 const renderScore=done=>{const group=document.createElement('span');group.className='t-digit-group';const digit=document.createElement('span');digit.className='t-digit';digit.textContent=String(done);group.append(digit);score.replaceChildren(group,document.createTextNode(' / '+questions.length));void group.offsetWidth;group.classList.add('is-animating')};
 const refresh=()=>{const done=questions.filter(question=>quickRecords[question.dataset.trainerQuestion]?.correct).length;renderScore(done);summary.textContent=done===questions.length?(trainer.dataset.trainerDone||'Глава закреплена. Можно переходить дальше.'):done?'Верно: '+done+' из '+questions.length+'. Завершите мини-тренажёр.':'Ответьте на оба вопроса.';trainer.classList.toggle('is-complete',done===questions.length);if(done===questions.length)stampTimeline('chapter:'+trainer.dataset.chapterTrainer)};
 questions.forEach(question=>{
  const id=question.dataset.trainerQuestion;
  const variants=[...question.querySelectorAll('[data-trainer-variant]')];
  const stem=question.querySelector('[data-trainer-stem]');
  const button=question.querySelector('[data-trainer-check]'),feedback=question.querySelector('[data-trainer-feedback]');
  const check=document.createElement('span');check.className='trainer-check';
  check.innerHTML='<svg viewBox="0 0 24 24" width="22" height="22" aria-hidden="true"><path d="M4 12.5 9.5 18 20 6.5"/></svg>';
  button.after(check);
  const saved=quickRecords[id];
  let shown=(Number(saved?.correct?saved.variant:saved?.attempts)||0)%variants.length;
  const show=index=>{shown=index;variants.forEach((box,order)=>{box.hidden=order!==index});stem.textContent=variants[index].dataset.stem};
  const inputs=()=>[...variants[shown].querySelectorAll('input[type=radio]')];
  const answerOf=index=>answerIndex(id+'/'+index,variants[index]);
  show(shown);
  const settle=correct=>{
   feedback.hidden=false;
   feedback.className='trainer-feedback '+(correct?'is-correct':'is-wrong');
   if(correct){
    check.classList.remove('is-shown');void check.offsetWidth;check.classList.add('is-shown');
    inputs().forEach(input=>input.disabled=true);button.disabled=true;button.textContent='Засчитано';
   }else{
    const shakeMs=motionMs('--shake-dur-a',80)*2+motionMs('--shake-dur-b',60)*2;
    question.classList.remove('is-shaking');void question.offsetWidth;question.classList.add('is-shaking');
    setTimeout(()=>question.classList.remove('is-shaking'),shakeMs+20);
    button.textContent='Проверить';
   }
  };
  if(saved?.correct){
   const answer=answerOf(shown);
   const labels=[...variants[shown].querySelectorAll('label')];
   inputs()[answer].checked=true;labels[answer]?.classList.add('is-correct');
   feedback.textContent='Ранее отвечено верно. '+variants[shown].dataset.explanation;
   settle(true);
  }
  button.addEventListener('click',()=>{
   const selected=inputs().find(input=>input.checked);
   if(!selected){feedback.hidden=false;feedback.className='trainer-feedback is-wrong';feedback.textContent='Сначала выберите вариант ответа.';return}
   const correct=Number(selected.value)===answerOf(shown);
   recordQuickCheck(id,correct,shown);
   if(correct){
    selected.closest('label')?.classList.add('is-correct');
    feedback.textContent='Верно. '+variants[shown].dataset.explanation;
    settle(true);
   }else{
    selected.closest('label')?.classList.add('is-wrong');
    const single=variants.length<2;
    feedback.textContent=single?'Пока неверно. Вернитесь к главе: разбор откроется после верного ответа.':'Пока неверно. Вот другой вопрос о том же — разбор откроется после верного ответа.';
    settle(false);
    if(!single){
     button.disabled=true;
     const next=(shown+1)%variants.length;
     setTimeout(()=>{
      variants[shown].querySelectorAll('input[type=radio]').forEach(input=>{input.checked=false});
      variants[shown].querySelectorAll('label').forEach(label=>label.classList.remove('is-correct','is-wrong'));
      show(next);button.disabled=false;
     },motionMs('--shake-dur-a',80)*2+motionMs('--shake-dur-b',60)*2+60);
    }
   }
   refresh();
  });
...
const scrollables=()=>{document.querySelectorAll('.prose pre, .prose table').forEach(box=>{
 const wide=box.scrollWidth>box.clientWidth+1;
 if(wide===box.hasAttribute('tabindex'))return;
 if(wide){box.setAttribute('tabindex','0');box.setAttribute('role','group');box.setAttribute('aria-label','Прокручиваемый блок: стрелки влево и вправо');}
 else{box.removeAttribute('tabindex');box.removeAttribute('role');box.removeAttribute('aria-label');}
});};
scrollables();
let scrollTimer=0;addEventListener('resize',()=>{clearTimeout(scrollTimer);scrollTimer=setTimeout(scrollables,150);});
...
// 120 000 слов до сих пор существовали только постранично: чтобы напечатать
// книгу или положить её в читалку, пришлось бы 40 раз нажать «печать». Здесь
// лежит вся проза подряд — вводные уроки, 34 главы, приложения, — и ничего
// сверх неё: мини-тренажёр, свободное воспроизведение и карточки практикума
// на бумаге бессмысленны, а без своего скрипта ещё и пусты.
const bookBody = [
  ...lessons.map((item) => item.intro + item.content),
  ...chapters.map((item) => item.intro + item.content),
  source.slice(appendicesStart, selftestStart),
].join('\n');
// Внутри книги ссылка на раздел книги обязана остаться местной: иначе печать
// даёт сноски, уводящие с листа на сайт. Всё, чего на этой странице нет
// (работы практикума, кабинет), по-прежнему ведёт на свою страницу.
const bookIds = new Set([...bookBody.matchAll(/\bid="([^"]+)"/g)].map((match) => match[1]));
const bookLinks = (html) => html.replace(/href="#([^"]+)"/g, (_, id) => {
  if (bookIds.has(id)) return `href="#${id}"`;
  const target = idToUrl.get(id);
  return target ? `href="${target}#${id}"` : `href="#${id}"`;
});
const bookLead = `<header class="catalog-head"><h1>Вся книга одной страницей</h1><p>Весь текст учебника подряд: пять вводных уроков, 34 главы и приложения. Отсюда его печатают, сохраняют в PDF (печать → «Сохранить как PDF») или кладут в читалку. Проверки, тренажёры и практикум сюда не попали — они живут на <a href="/assessment/">своей странице</a> и работают только в браузере.</p></header>`;
const bookPage = pageShell({
  title: 'Вся книга одной страницей',
  eyebrow: 'Для печати, PDF и чтения без сайта',
  description: 'Полный текст учебника по серверной инфраструктуре одной страницей: вводные уроки, 34 главы и приложения — для печати, PDF и чтения без сайта.',
  body: `${bookLead}<article class="prose book-prose">${bookLinks(bookBody)}</article>`,
  rail: false,
  className: 'book-page',
});
...
for (const [url, html] of outputs) if (url !== '/book/') indexPage(url, html);
