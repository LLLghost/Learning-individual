...
login_hint() {
  ...
  say '  мосты стенда подняты без портов: маршрута в сети узлов у хоста нет, и'
  say '  снаружи к ним не подойти. Консоль машины в веб-интерфейсе Proxmox'
  say '  работает всегда — с неё и начинайте.'
  # Облачный образ Debian принимает по SSH только ключ: на собранном стенде
  # router отвечает «Permission denied (publickey)», даже когда пароль задан и
  # в консоли работает. Человек, задавший один STAND_PASSWORD, ищет тогда
  # причину в пароле — а она в способе входа, и по выводу этого не видно.
  if [ -z "$SSHKEYS" ]; then
    say '  по SSH в гостей не войти: образ принимает только ключ, а ключ не задан.'
    say '  Пароль работает в консоли. Ключ задаётся при сборке — STAND_SSHKEYS'
  elif [ "$WAN_MODE" = nat ]; then
    # В режиме трансляции хост стоит с router в одной сети, и это единственная
    # дверь в стенд снаружи: с самого router видны остальные узлы.
    say "  по SSH с хоста доступен только router — ssh $CIUSER@$NAT_NET.2,"
    say '  дальше по стенду с него же'
  else
    say '  по SSH: внешний адрес router выдаёт DHCP управляющей сети, посмотреть'
    say "  его можно так — qm guest cmd $(vmid_of 0) network-get-interfaces"
  fi
  local node
  for node in "${STAND_NODES[@]}"; do
