...
# Отдельный каталог с одним только id: им изображается машина, где Proxmox нет
# вовсе. Права при этом остаются подставными — иначе проверка прав срабатывает
# раньше и случай проверяет не то, что написано в его имени.
mkdir -p "$WORK/bare"
cp "$WORK/bin/id" "$WORK/bare/id"
mkdir -p "$WORK/pve"
...
# Случай о машине, где Proxmox не установлен. Права подставляются намеренно:
# без этого на любом хосте, где прогон идёт не от root, скрипт останавливается
# на проверке прав, ожидаемая строка не появляется, и случай красный не потому,
# что дефект есть, а потому, что до проверяемого отказа он не доходит. Так и
# вышло: под root проходило, на раннере — нет.
case_name 'машина не Proxmox: скрипт говорит об этом, а не падает'
LOG="$WORK/calls.log"; : >"$LOG"
OUT=$(env PATH="$WORK/bare:/usr/bin:/bin" STUB_LOG="$LOG" bash "$STAND" status 2>&1) || true
